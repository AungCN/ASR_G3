...
> **This folder's canonical entry point is `recorder.py` (= version 0.91, LF line endings).**
> The tagged originals are kept in [`old_versions/`](old_versions/). Everything below
> refers to `recorder.py`.
>
> **Install:** `pip install PyQt6 sounddevice numpy` — on macOS also `brew install portaudio`
> (the `sounddevice` wheel links against the PortAudio system library). See
> [`../SETUP.md`](../SETUP.md) for microphone-permission steps.
>
> **Known limitation:** `utt_id` is `speaker_YYYYMMDD_HHMMSS`, so two *Save* presses
> within the same second collide (the second overwrites the WAV and appends a
> duplicate line). Wait ~1 s between saves; `tools/validate_recording.py` flags any
> duplicate that slips through.

<p align="center">
<img src="recorder_v0.9.png" alt="UI" width="500"/>  
</p>  
<div align="center">
...
> **Paths in `wav.scp` are absolute paths on the recording machine** (see the
> Windows example below) and will not resolve after the corpus is transferred.
> Regenerate them on the training box with `tools/merge_data_dir.py` (portable,
> pure-Python) or Kaldi's own `wav.scp` rewrite step. The recorder also *appends*
> to `wav.scp`/`text`/`utt2spk` without sorting or de-duplicating — `merge_data_dir.py`
> handles both.
